...
class environment5:
    def __init__(self):
        ...
        self.action_space = {'same-scatterplot-0-1':0, 'same-bar-2':1, 'same-bar-4':2, 'same-hist-3':3, 'modify-scatterplot-0-1':4, 'modify-bar-2':5, 'modify-bar-4':6, 'modify-hist-3':7}
        self.steps = 0
    ...
    def process_data(self, task, raw_file, feedback_file, thres, algo):
        ...
        for d in data:
            ...
            state = self.get_state(task, d[3], d[4], algo)
            self.mem_states.append(state)
            if d[5] == 1:
                self.mem_reward.append(d[5])
            else:
                self.mem_reward.append(0.2)

            # print(d[1])
            self.mem_action.append(self.action_space[d[6]])
